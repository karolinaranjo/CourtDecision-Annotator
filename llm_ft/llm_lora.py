...
class LLM_Lora(object):
    """ Base class for LoRA fine-tuning
    """
    def __init__(self,
                 base_model: str = "",
    ...
    def predict(self,
        ...
        # Load the base model
        self.load_base_model()
        self.prompter = Prompter(kshot=kshot, verbose=False, cutoff_length=self.cutoff_length)
        ...
        for i in tqdm(range(len(test_list))):
            ...
            # print(item['input'])
            # # print('ANSWER!!!!',res,len(res))
            text = res 
            # #text = res[0] # [item for item in res][0]
    ...
    def generate_and_tokenize_prompt(self, data_point):
        ...
        #     )
        #     tokenized_user_prompt = self.tokenize(user_prompt, add_eos_token=self.add_eos_token)
        #     user_prompt_len = len(tokenized_user_prompt["input_ids"])

        #     if self.add_eos_token:
        #         user_prompt_len -= 1
        #     # ignore computing the loss of input tokens by setting their id to -100
